...
<div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
    <div class="px-8 py-6 bg-slate-50/50 border-b flex gap-4">
        <form method="GET" action="{{ route('admin.partners.index') }}" class="relative flex-1 flex gap-3">
            ...
            @if($search)
            <a href="{{ route('admin.partners.index') }}" class="px-6 py-3 border border-slate-200 text-slate-600 rounded-xl font-bold hover:bg-slate-50 transition">
                Reset
            </a>
            @endif
        </form>
            ...
            <tbody class="divide-y border-t">
                ...
                <tr>
                    <td colspan="6" class="px-8 py-16 text-center text-slate-400 font-medium">
                        @if($search)
                            Tidak ada partner yang cocok dengan pencarian "<strong>{{ $search }}</strong>".
                        @else
