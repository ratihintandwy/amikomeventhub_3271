    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden mb-6">
        <div class="px-8 py-6 bg-slate-50/50 border-b flex gap-4">
            <form method="GET" action="{{ route('admin.categories.index') }}" class="relative flex-1 flex gap-3">
                ...
                @if(request('search'))
                <a href="{{ route('admin.categories.index') }}" class="px-6 py-3 border border-slate-200 text-slate-600 rounded-xl font-bold hover:bg-slate-50 transition flex items-center justify-center">
                    Reset
                </a>
                @endif
            </form>
